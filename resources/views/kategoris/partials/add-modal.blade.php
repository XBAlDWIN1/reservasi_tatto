    ...
    <div class="bg-white rounded-lg p-6 w-full max-w-md" @click.away="addModal = false">
        ...
        <form method="POST" action="{{ route('kategoris.store') }}">
            ...
            <div class="mb-4">
                ...
                @error('nama_kategori') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
            </div>
            <div class="mb-4">
                <label class="block mb-1">Deskripsi</label>
                <input type="text" name="deskripsi" value="{{ old('deskripsi') }}" class="w-full border rounded px-3 py-2">
